...
const RestaurantDetails: NextPage = () => {
    ...
    const reviewsQuery = api.restaurants.getReviews.useQuery({ id: id as string });
    const setReviewMutation = api.restaurants.setReview.useMutation();
    const session = useSession()

    useEffect(() => {
        if(reviewsQuery.data && reviewsQuery.data._avg.rating) setRating(reviewsQuery.data._avg.rating);
    ...
    function setRating(rating: number) {
        void setReviewMutation.mutateAsync({ id: id as string, rating: rating }).then(()=>{
    ...
    return (
        <>
            <main className="flex min-h-screen flex-col items-center gap-y-5 bg-primary-foreground p-5 md:items-start">
                ...
                <Rating value={reviewsQuery.data._avg.rating || 0} onChange={setRating} className="">
                    <Rating.Item name="rating-1" disabled={session.status==='unauthenticated'} className="mask mask-star bg-orange-400" />
                    <Rating.Item name="rating-1" disabled={session.status==='unauthenticated'} className="mask mask-star bg-orange-400" />
                    <Rating.Item name="rating-1" disabled={session.status==='unauthenticated'} className="mask mask-star bg-orange-400" />
                    <Rating.Item name="rating-1" disabled={session.status==='unauthenticated'} className="mask mask-star bg-orange-400" />
                    <Rating.Item name="rating-1" disabled={session.status==='unauthenticated'} className="mask mask-star bg-orange-400" />
                ...
                <Map
                        position={[restaurantsQuery.data.lat, restaurantsQuery.data.lng]}
                ...
                <p>Sito web: <Link className="underline" href={restaurantsQuery.data.website}>{restaurantsQuery.data.website}</Link></p>
                ) : null}
            </main>
        </>
    );
